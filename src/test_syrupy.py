...
def test_do_ops_with_protocol_mock(mocker: MockerFixture, snapshot):
...
def test_do_ops_DI_with_protocol_mock(mocker: MockerFixture, snapshot):
    ...
    print_spy = mocker.patch(
        "examples.basics.RealOpsOne.concatenation", wraps=real_ops.concatenation
    )
...
import src.capture.capture as c  # noqa
...
def test_do_ops_DI_with_protocol_mock_snap(mocker: MockerFixture, snapshot):  # noqa: C901
    ...
    if len(table_tests) == 0:
        raise ValueError(
            "No captures found for do_ops_DI - first run your production with SNAPY_CAPTURE_ENABLED=1"
        )
    ...
    for blob_path in table_tests:
        ...
        def plus_mock(*args, **kwargs):
            ...
            side_effect_target_path = c.side_effect_target_path(*test_specifier, plus_mock)  # noqa: B023
            ...
            returned, was_found = c.side_effect_lookup(
                args,
                kwargs,
                side_effect_target_path,
                in_test_mode=os.getenv("SIDE_EFFECT_TEST_MODE") == "1",
            )
            if was_found:
                return returned
            ...
            @c.capture(
                ...
                target_path=side_effect_target_path,
                in_capture_mode=os.getenv("SNAPY_CAPTURE_ENABLED") == "1",
            )
        ...
        def expression_mock(*args, **kwargs):
            ...
            side_effect_target_path = c.side_effect_target_path(*test_specifier, expression_mock)  # noqa: B023
            returned, was_found = c.side_effect_lookup(
                args, kwargs, side_effect_target_path, in_test_mode=TM()
            )
            if was_found:
                return returned

            @c.capture(
                max_captures=float("inf"), target_path=side_effect_target_path, in_capture_mode=CE()
            )
        ...
        def concatenation_mock(*args, **kwargs):
            ...
            side_effect_target_path = c.side_effect_target_path(*test_specifier, concatenation_mock)  # noqa: B023
            returned, was_found = c.side_effect_lookup(
                args, kwargs, side_effect_target_path, in_test_mode=TM()
            )
            if was_found:
